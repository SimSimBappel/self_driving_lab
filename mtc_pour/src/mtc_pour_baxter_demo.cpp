...
int main(int argc, char **argv) {
...

  {
    auto stage = std::make_unique<stages::Connect>(
        ...
        stages::Connect::GroupPlannerVector{{"right_arm", sampling_planner}});
...

  {
    auto stage = std::make_unique<stages::MoveRelative>("approach object",
                                                        cartesian_planner);
    stage->properties().set("marker_ns",
                            "approach"); // TODO: convenience wrapper
...

  {
    ...
    wrapper->setIKFrame(
        Eigen::Translation3d(0.02, 0, -0.05) *
            Eigen::AngleAxisd(M_PI / 2 + .7, Eigen::Vector3d::UnitY()),
        "right_gripper");
...

  {
    ...
    wrapper->setMaxIKSolutions(32);
    wrapper->setIKFrame(Eigen::Translation3d(-0.00, 0, -0.02) *
...

  {
    auto stage = std::make_unique<stages::MoveRelative>("put down object",
                                                        cartesian_planner);
    stage->properties().set("marker_ns",
                            "approach_place"); // TODO: convenience wrapper
    stage->properties().set("link", "right_gripper");
    stage->properties().configureInitFrom(Stage::PARENT, {"group"});
    stage->setMinMaxDistance(.08, .13);
...

  {
    ...
    auto wrapper = std::make_unique<stages::ComputeIK>("place pose kinematics",
                                                       std::move(stage));
    wrapper->setMaxIKSolutions(32);
    wrapper->setIKFrame(Eigen::Translation3d(0.02, 0, -0.05) *
                            Eigen::AngleAxisd(.7, Eigen::Vector3d::UnitY()),
                        "right_gripper");
